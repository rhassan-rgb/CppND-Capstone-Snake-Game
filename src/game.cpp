...
void Game::Run(Controller const &controller, Renderer &renderer,
    ...
    std::function<void(KeyStroke)> callback =
        std::bind(&Snake::Control, snake.get(), std::placeholders::_1);

    std::future<void> controlHandler =
        std::async(&Controller::InputListener, &controller, running);
    std::future<void> inpHandler = std::async(
        std::launch::async,
        [&controller, &callback]() { controller.InputHandler(callback); });
