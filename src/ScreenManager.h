...
class ScreenManager : public Renderer {
   public:
    ...
    ScreenManager(const std::size_t screen_width,
...
class SelectionMenu {
   public:
    ...
    void nextItem() { _current = (_current + _size + 1) % _size; }
    int getSelected() { return _current; }
    void previousItem() { _current = (_current + _size - 1) % _size; }
