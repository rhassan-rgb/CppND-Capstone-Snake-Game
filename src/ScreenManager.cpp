...
ScreenManager::ScreenManager(const std::size_t screen_width, const std::size_t screen_height,
           const std::size_t grid_width, const std::size_t grid_height) :  Renderer(screen_width, screen_height, grid_width, grid_height)
           {
...
void ScreenManager::drawText(const std::string& text, int x, int y, bool selected) {
    SDL_Color textColor = selected ? _selectedColor : _normalColor;
    SDL_Surface* surface = TTF_RenderText_Solid(_gFont, text.c_str(), textColor);
...
void ScreenManager::handleTextInput(const SDL_TextInputEvent& e, std::string& playerName) {
...
void ScreenManager::renderWelcomeScreen(WelcomeItems selected)
{
    ...
    drawText("Start a new game", 150, 200, WelcomeItems::ITEM_NEW_GAME == selected);

    drawText("Leader board", 150, 260, WelcomeItems::ITEM_LEADER_BOARD == selected);
...
Screens handleScreenAction(Screens currentScreen, int selectedItem)
{
    ...
    case Screens::SCREEN_WELCOME:
        ...
        case WelcomeItems::ITEM_EXIT:
            // returning retVal = Screens::TOTAL_ITEMS will cause the program to terminate;
        default:
            break;
        }
        break;
